...
"#).unwrap();
    for seed in 0..20u64 {
        let out = run_with_seed(&p, seed);
        assert!(out.len() >= 1 && out.len() <= 30, "seed {} len {} got {:?}", seed, out.len(), out);
        assert!(out.chars().all(|c| c == 'y' || c == 'x'), "seed {} got {:?}", seed, out);
    }
}
...
fn op_repeat_inline_recalculated() {
    ...
    let out = run_with_seed(&p, 0);
    assert_eq!(out.matches("x ").count(), 3, "each of 3 evals should output 'x <option>', got {:?}", out);
    assert!(out.contains("aa") || out.contains("bb") || out.contains("cc"), "got {:?}", out);
}
