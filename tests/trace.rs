...
fn trace_literal_single_span() {
    ...
    assert_eq!(out, "hello");
    assert_eq!(trace.len(), 1, "single option branch should produce exactly one span");
...
"#).unwrap();
    ...
    assert_eq!(trace.len(), 1, "branch should produce exactly one span (chosen child only)");
...
"#).unwrap();
    ...
    assert_eq!(trace[0].start_line, 4, "trace must be only the chosen line (poka on line 4)");
