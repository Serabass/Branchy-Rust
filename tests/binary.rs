...
fn compile_then_run_equals_source_run() {
    ...
    let compile_ok = Command::new(bin)
        .args(["compile", source.to_str().unwrap(), "-o", binary.to_str().unwrap()])
        .output()
        .unwrap();
    assert!(compile_ok.status.success(), "compile failed: {:?}", String::from_utf8_lossy(&compile_ok.stderr));
    let run_binary = Command::new(bin)
        .args(["run", binary.to_str().unwrap()])
        .output()
        .unwrap();
    assert!(run_binary.status.success(), "run .branchyc failed: {:?}", String::from_utf8_lossy(&run_binary.stderr));
    let out_binary = String::from_utf8_lossy(&run_binary.stdout).trim().to_string();
    let run_source = Command::new(bin)
        .args(["run", source.to_str().unwrap()])
        .output()
        .unwrap();
    assert!(run_source.status.success(), "run .branchy failed: {:?}", String::from_utf8_lossy(&run_source.stderr));
    let out_source = String::from_utf8_lossy(&run_source.stdout).trim().to_string();
