...
fn eval_node(
  ...
  match node {
    ...
    Node::SpreadParam { .. } | Node::SpreadInclude { .. } => Err(err_span(
    ...
    Node::BinaryOp {
      ...
      BinOp::Star => {
        let n = match right.as_ref() {
          crate::ast::Node::Leaf {
            lit: crate::ast::Literal::Range(lo, hi),
            ..
          } => {
            let (lo, hi) = ((*lo).max(0), (*hi).max(0));
            if hi < lo {
    ...
    Node::Call {
      ...
      if let Some(ref blk) = block {
        for (var, value_node) in &blk.bindings {
          let s = eval_node(value_node, program, builtins, env, rng, block_nodes, trace)?;
