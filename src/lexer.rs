use crate::ast::{SourceError, span_from_offsets};
...
fn tokenize_range(input: &str, start: usize, end: usize) -> Result<Vec<(Token, usize, usize)>, SourceError> {
    ...
    while let Some((off, c)) = cur.next() {
        ...
        match c {
            ...
            '[' => {
                ...
                loop {
                    let (off2, c2) = match cur.next() {
                        Some(p) => p,
                        None => return Err(err_at(input, token_start, cur.position(), "unclosed [ (expected ] or ;)")),
                    };
                    content_end = off2;
                    match c2 {
                        '[' => depth += 1,
            ...
            '.' => {
                let (off2, c2) = cur.next().ok_or_else(|| err_at(input, token_start, cur.position(), "expected .. or ..."))?;
                if c2 != '.' {
                    return Err(err_at(input, token_start, off2 + c2.len_utf8(), "expected .. or ..."));
                }
                let (end_off, tok) = match cur.peek() {
                    ...
                    None => return Err(err_at(input, token_start, cur.position(), "expected .. or ...")),
                };
            ...
            ':' => {
                ...
                let (name_start, first_c) = cur.next().ok_or_else(|| err_at(input, token_start, token_end, "expected identifier after ':'"))?;
                if !is_ident_start(first_c) {
                    return Err(err_at(input, name_start, name_start + first_c.len_utf8(), "invalid param name"));
                }
            ...
            _ => return Err(err_at(input, token_start, token_end, format!("unexpected character: {}", c))),
        }
    }
    Ok(out)
}

fn read_quoted_offset(cur: &mut OffsetCursor<'_>, end: char, start: usize, input: &str) -> Result<(String, usize), SourceError> {
...
fn read_ident_from_offset(first: char, cur: &mut OffsetCursor<'_>, start: usize) -> (String, usize) {
    let mut s = String::from(first);
    let mut last_end = start + first.len_utf8();
    while let Some((off, c)) = cur.peek() {
        if is_ident_cont(c) {
...
mod tests {
    ...
    fn tokenize_spread_param_top_level() {
        ...
        assert_eq!(tokens, [Token::Spread, Token::Param("x".into())], "top-level ...:x");
    }

    #[test]
    fn tokenize_with_offsets_matches_tokenize() {
        for input in ["[ a; b; ]", "!f(:a) = [ x; ]; [ y; ]", r#"[ a; ...:x; b; ]"#] {
